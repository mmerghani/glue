...
(function cloudCliLauncher() {
  ...
  CC.act = function (name, node) {
    switch (name) {
      ...
      case 'dashboard':
        return CC.run('Opening Glue dashboard...', function () { return bridge.openCloudDashboard(); });
  ...
  CC.titlebar = function (state) {
    ...
    return '<div class="titlebar">' +
      '<div class="brand"><img class="mk" src="' + esc(LOGO_URL) + '" alt=""><span>Glue</span></div>' +
      '<div class="tb-tabs no-drag">' + renderTabs(state) + '</div>' +
      '<span style="flex:1"></span>' +
      refreshAction +
      envActions +
      '<button class="btn sm tb-action no-drag" data-cc-action="connect" title="' + esc(authState(state) === 'expired' ? 'Reconnect your Glue account' : accountLabel(state)) + '"><span class="dot" style="background:' + (conn ? 'var(--ok)' : (authState(state) === 'expired' ? 'var(--warn)' : 'var(--tx3)')) + '"></span>' + esc(accountLabel(state)) + '</button>' +
      logoutAction +
  ...
  CC.buildLocalServerSection = function (state, options) {
    ...
    return CC.renderSection(
      ...
      options.title || 'Run Local Glue on this machine',
      body
    );
  };
  ...
  CC.renderLocalSettings = function () {
    ...
    var sections = [
      ...
      CC.renderSection('PREFERENCES', 'How the local service behaves', '' +
        ...
        '<label class="cc-toggle"><input type="checkbox" data-cc-setting="keepLocalServerRunning"' + ((state.desktopSettings || {}).keepLocalServerRunning ? ' checked' : '') + '><span><b>Keep server running</b><br>Leave Local Glue available after you quit the app.</span></label>' +
    ...
    CC.renderSheet('Local Settings', 'Manage how Local Glue runs on this computer.', sections);
  };
...
(function sidebarApp() {
  ...
  function cloudPane(state) {
    ...
    if (CC.authState(state) === 'expired') {
      return header + '<div class="empty">Your Glue session expired.<div style="margin-top:14px"><button class="btn pri" data-cc-action="connect">' + CC.icon('cloudPlus', 15) + 'Reconnect account</button></div></div>';
    }
    if (!CC.connected(state)) {
      return header + '<div class="empty">Connect your Glue account to list hosted environments.<div style="margin-top:14px"><button class="btn pri" data-cc-action="connect">' + CC.icon('cloudPlus', 15) + 'Connect account</button></div></div>';
    }
    if (state.cloudLoading && !(state.environments || []).length) {
      return header + '<div class="empty">Loading your Glue environments...</div>';
    }
