...
const APP_NAME = 'Glue';
const APP_USER_MODEL_ID = 'ai.glue.desktop';
...
async function copyDiagnostics() {
  ...
  await dialog.showMessageBox(desktopWindow?.getMainWindow() || undefined, {
    ...
    message: 'Glue desktop diagnostics were copied to the clipboard.',
  });
}
...
async function refreshCloudEnvironments({ showErrors = false } = {}) {
  ...
  try {
    ...
    if (authState === 'expired') {
      const expiredError = new Error('Your Glue session expired. Reconnect your account.');
      if (showErrors) {
        await showError('Glue login required', expiredError);
        return [];
      }
      throw expiredError;
    }
    if (showErrors) {
      await showError('Could not load Glue environments', error);
      return [];
    }
    throw error;
  } finally {
...
async function handleDeepLink(url) {
  ...
  if (!pendingCloudConnectStartedAt || Date.now() - pendingCloudConnectStartedAt > AUTH_CALLBACK_TTL_MS) {
    await showError('Glue account connection failed', new Error('No recent Glue account connection was started from this app.'));
    return;
  }

  const apiKey = parsed.searchParams.get('api_key');
  if (!apiKey) {
    await showError('Glue account connection failed', new Error('The callback did not include an API key.'));
    return;
  }
  ...
  dialog.showMessageBox(desktopWindow?.getMainWindow() || undefined, {
    type: 'info',
    title: 'Glue account connected',
    message: cloud.getAccount()?.email ? `Connected as ${cloud.getAccount().email}.` : 'Glue account connected.',
  }).catch(() => {});
}
...
async function copyLocalWebUrl() {
  ...
  await dialog.showMessageBox(desktopWindow?.getMainWindow() || undefined, {
    ...
    detail: isLanUrl
      ...
      : `${shareableUrl}\n\nThis URL works on this computer. Enable LAN access before starting Local Glue to copy a phone-accessible URL.`,
  });
...
async function updateDesktopSetting(key, value) {
  ...
  if (result.requiresRestartNotice) {
    await dialog.showMessageBox(desktopWindow?.getMainWindow() || undefined, {
      ...
      detail: 'Quit Glue and stop the local server, then open Local Glue again.',
    });
  }
...
async function showEnvironmentPicker() {
  ...
  const choices = ['Local Glue', ...environments.map((environment) => {
...
async function openEnvironmentInDesktop(environment) {
  ...
  if (environment.status !== 'running') {
    const response = await dialog.showMessageBox(desktopWindow?.getMainWindow(), {
      ...
      detail: 'Glue can start it before opening the remote app.',
    });
...
function getRemoteEnvironmentMenuItems() {
  ...
  if (!cloudAccount?.apiKey) {
    return [{ label: 'Connect Glue Account...', click: () => void connectCloudAccount() }];
  }
...
if (registerSingleInstance()) {
  bootstrap().catch(async (error) => {
    await showError('Glue failed to start', error);
    app.quit();
  });
}
