...
export class DesktopWindowManager {
      ...
      return [
        {
          ...
          click: () => void this.actions.connectCloudAccount()
            .catch((error) => this.actions.showError('Could not connect Glue account', error)),
        },
      ];
    }
  ...
  buildAppMenu() {
    ...
    const cloudAccountLabel = cloudState.account?.apiKey
      ? (cloudState.account?.email ? `Connected: ${cloudState.account.email}` : 'Glue Connected')
      : (cloudState.account?.email ? `Reconnect: ${cloudState.account.email}` : 'Connect Glue Account...');
    ...
    const template = [
      ...
      },
      {
        ...
        submenu: [
          ...
          { type: 'separator' },
          {
            label: 'Open Local Glue',
            accelerator: 'CmdOrCtrl+L',
            click: () => void this.actions.openLocalInDesktop().catch((error) => this.actions.showError('Could not open local Glue', error)),
          },
          {
        ...
        submenu: [
          {
            ...
            click: () => void this.actions.connectCloudAccount().catch((error) => this.actions.showError('Could not connect Glue account', error)),
          },
          {
            label: 'Refresh Cloud Environments',
            click: () => void this.actions.refreshCloudEnvironments().catch((error) => this.actions.showError('Could not load Glue environments', error)),
            enabled: Boolean(cloudState.account?.apiKey),
          },
          {
            label: 'Logout Glue Account',
    ...
    const template = [
      {
        ...
        submenu: [
          {
            label: localState.localServerRunning ? 'Open Local in Glue' : 'Start Local in Glue',
            click: () => void this.actions.openLocalInDesktop().catch((error) => this.actions.showError('Could not open local Glue', error)),
          },
          {
